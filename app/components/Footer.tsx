...
function ContactInfo({ name, phone }: { name: string; phone: string }) {
  ...
  return (
    <a href={`tel:${cleanNumber}`} className="flex items-center gap-3 group">
      <div className="w-7 h-7 flex-shrink-0 rounded-full bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-white transition-all duration-200">
        <FaPhoneAlt size={11} className="text-gray-400 group-hover:text-black" />
      </div>
      <div>
        <p className="text-xs font-bold tracking-wide text-white/90 leading-tight group-hover:text-white">
          {name}
        </p>
